...
from src.models.deep_learning import tune_attention_lstm_regressor


def main():
    """
    We tune the Attention-LSTM on a controlled subset of the data.
    """
    ...
    # We create the same feature set as the plain LSTM run. The attention model
    # should be compared on the same inputs.
    ...
    # We use fewer symbols than the plain LSTM because attention is heavier and
    # each hyperparameter combination takes longer to train.
    n_symbols_for_tuning = 300

    # We keep symbol selection deterministic so repeated tuning runs are easier
    # to compare.
    ...
    print("\nAttention-LSTM tuning dataset created.")
    ...
    print("\nStarting Attention-LSTM hyperparameter tuning...")

    tuning_results = tune_attention_lstm_regressor(
        train_df=train_df,
    ...
    print("\nAttention-LSTM hyperparameter tuning completed.")
    print("\nBest configurations:")
    print(tuning_results.head(10))


    output_path = "models/attention_lstm_regression_hypertuning_results.csv"
    ...
    print("\nBest Attention-LSTM configuration selected:")
    print(best_config)

    print("\nUse this best configuration later for full-data Attention-LSTM training:")
